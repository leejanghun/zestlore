...
class Widget_tour_list extends Widget {
	...
	public function run( $arrayData )
	{
		...
		{
            $arr_sql_where[] = " tour_200_default.".$arr_VARIABLE_DATA['FILTER_FIELD_NAME'] . " Like '%" . $arr_VARIABLE_DATA['FILTER_FIELD_VALUE'] . "%'";
		...
		$arr_SQL_Result	=   null;
		$arr_SQL_Result       =   $this->CI->Tour_model->getTour_RecordCnt( $arrData );
		...
		$arrData['OrderBy']        =   "tour_200_pk desc";
		...
		for($loopCnt_1 = 0; $loopCntLimit > $loopCnt_1; $loopCnt_1++)
		{
			...
			$arr_recordList[$loopCnt_1]['detail_url']   .=  "index.php/tour_default/tour_detail";
			$arr_recordList[$loopCnt_1]['detail_url']   .=   "?tour_200_pk=".$arr_SQL_Result[$loopCnt_1]['tour_200_pk'];
			...
			if($arr_SQL_Result[$loopCnt_1]['tour_200_firstimage2'])
			{
				$arr_recordList[$loopCnt_1]['tour_image'] = $arr_SQL_Result[$loopCnt_1]['tour_200_firstimage2'];
			}
		...
		$arr_FILTER_FIELD_OPTION_TEXT		=	array("제목", "주소");
		$arr_FILTER_FIELD_OPTION_VALUE		=	array("tour_200_title", "tour_200_addr1");
		...
		$htmlFilePath         =   "widgets/tour/widget_tour_list.html";
