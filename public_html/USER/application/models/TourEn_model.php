...
class TourEn_model extends CI_Model
{
	...
	public function getTourEn_RecordCnt( $arrData )
	{
		...
		$this->db->from( "tourEn_300_default" );
...
//         	        echo "<pre>";
	...
	public function getTourEn_RecordList( $arrData )
	{
		...
		$this->db->select("tourEn_300_default.*", FALSE);
	...
	public function update_tourEn_300_default( $arrData )
	{
		...
		try
		{
			...
			$this->db->where('tourEn_300_pk', $arrData['arr_data1']['tourEn_300_pk']);
			...
			if( $db_error["code"] == 0 )
			{
				...
				$rtnValue["SQL_Result_Data"]['tourEn_300_pk'] =   $arrData['arr_data1']['tourEn_300_pk'];
			}else{
