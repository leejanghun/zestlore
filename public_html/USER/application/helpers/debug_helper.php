...
// debug_backtrace() 함수의 간략버전
function simple_debug_backtrace()
{

	$cnt = count( debug_backtrace() );
	for($i=0; $i<$cnt; $i++)
	{
		@$error_pnt[$i]['class'] = debug_backtrace()[$i]['class'];
		@$error_pnt[$i]['fun'] = debug_backtrace()[$i]['function'];
		@$error_pnt[$i]['line'] = debug_backtrace()[$i]['line'];
		//debug_var( $error_pnt );
	}
